...
def append_float16(buffer: bytearray, value: float, scale: float, index: int) -> int:
    return append_int16(buffer, int(value * scale), index)


def append_float32(buffer: bytearray, value: float, scale: float, index: int) -> int:
    return append_int32(buffer, int(value * scale), index)


def append_float64(buffer: bytearray, value: float, scale: float, index: int) -> int:
    return append_int64(buffer, int(value * scale), index)
